...
- **Identical entities short-circuit.** `if *self == other { return self; }`. This is **not** an optimization: the rule above removes an update equal to `created_at`, so without the guard an entity parsed from `LAST_MODIFIED == ADD_DATE` - the `html/bookmarks_simple` shape - would lose that update on meeting a byte-identical duplicate. No fixture covers it, because no fixture duplicates an anchor carrying `LAST_MODIFIED`; `merge_is_idempotent_for_identical_entities` is what pins it, as its counterparts do in hbt-go and hbt-ocaml.
