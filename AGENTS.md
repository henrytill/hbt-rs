...
- **Absence has a wire representation**, which is what makes the rule survive a round-trip: `createdAt` is omitted when absent, as `shared` and `lastVisitedAt` are, and is no longer in the schema's `required` list. Before that, an undated entity serialized as `createdAt: 0` and decoded back as one genuinely created on 1970-01-01, so the same collection merged differently depending on whether it had passed through YAML. A real creation time of `0` still serializes, and still exports `ADD_DATE="0"` -- the HTML template tests `is not none`, not truthiness, which is the trap `last_modified` still falls into (#63).
